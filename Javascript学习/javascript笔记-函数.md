...
有时，需要在定义函数之后，立即调用函数。但是不能在函数定义之后加圆括号()。否则会报错。

js引擎规定，如果function关键字出现在行首，一律解释称语句。所以认为这一段都是函数的定义，不应该以圆括号结尾，所以就报错了。

解决办法就是不要让function出现在行首，让引擎理解成一个表达式。一般会把它放圆括号里。
...
eval没有自己的作用域，因此可能会修改当前作用域变量的值，造成安全问题。
...
由于安全风险和不利于 JavaScript 引擎优化执行速度，所以一般不推荐使用。通常情况下，**`eval`最常见的场合是解析 `JSON` 数据的字符串，不过正确的做法应该是使用原生的`JSON.parse`方法。**
